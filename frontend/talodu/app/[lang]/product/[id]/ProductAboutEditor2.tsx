...
const ProductAboutsEditor = ({ productId, languages,initialDetails }: { 
    
    abouts: any[], 
    productId: number,
    languages: string[],
    initialDetails: ProductAbout[] 
}) => {
  ...
  const [showTranslationForm, setShowTranslationForm] = useState(false);
    ...
    const handleSaveTranslation = async (translation: any) => {
        try {
            const response = await fetch(`/api/products/${productId}/abouts/${translation.product_about_id}/translations`, {
                method: 'POST',
                headers: {
